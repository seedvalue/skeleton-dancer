...
public class BeatDetectionExample : MonoBehaviour
{
	...
	public List<AudioSource> m_MusicList;




	public beatmode beatMode;
	//We support energy, frequency and both, for beat detection

	//MAGIC NUMBERS. USE WITH CAUTION
	private int numSamples = 1024;
	//Num samples to precess MUST be a power of 2
	...
	private void StartMusic ()
	{
		...
		if (PlayerPrefs.HasKey ("MusicNum") == true) {
			_num = PlayerPrefs.GetInt ("MusicNum");
		}
		GetComponent<AudioSource> ().clip = m_MusicList [_num].clip;
		GetComponent<AudioSource> ().Play ();
		//	m_MusicList [_num].Play ();
	...
	********************************************/
	void Update ()
	{
		...
		*/
		...
		if ((theBeat & (int)beatType.kick) != (int)beatType.None) {
			...
			StartCoroutine (ShowBeatObj (m_Kick));

		}
		if ((theBeat & (int)beatType.snare) != (int)beatType.None) {
			//StartCoroutine (showText (snare, gsnare));
			StartCoroutine (ShowBeatObj (m_Snare));

		}
		if ((theBeat & (int)beatType.hithat) != (int)beatType.None) {
			//StartCoroutine (showText (hithat, ghithat));
			StartCoroutine (ShowBeatObj (m_HitHat));

		}
		if ((theBeat & (int)beatType.energy) != (int)beatType.None) {
			//StartCoroutine (showText (energy, genergy));
			StartCoroutine (ShowBeatObj (m_Energy));
		}
	...
	public GameObject m_HitHat;
	public GameObject m_Energy;
	public GameObject m_Snare;
	public GameObject m_Kick;
	...
	private IEnumerator ShowBeatObj (GameObject _beatObj)
	{
		SetObjectShow (_beatObj, true);
		yield return new WaitForSeconds (MIN_BEAT_SEPARATION * 3);
		SetObjectShow (_beatObj, false);
		yield break;
	}
	...
	private void SetObjectShow (GameObject _obj, bool _isshow)
	{
		if (_obj) {
